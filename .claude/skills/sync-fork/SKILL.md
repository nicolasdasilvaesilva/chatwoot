...
### B) indica-facil/chatwoot → indica-facil/chatwoot-pro (Pro merge)

Switch to `chatwoot-pro-main`, pull it even with `chatwoot-pro/main`, then `git merge main --no-ff -m "Merge branch 'main' into chatwoot-pro-main"`. Repo history shows this is done directly on `chatwoot-pro-main` (no PR), then pushed to `chatwoot-pro/main` along with the new `vX.Y.Z-indica-facil-pro.N` tag.
...
7. For Pro merges, recall that pushing to `chatwoot-pro/main` is directly followed by tagging `vX.Y.Z-indica-facil-pro.N` and cutting a release — coordinate with the `release-notes` skill (and its `PRIVACY.md` companion) before writing the release body.
...
git remote -v               # should show `chatwoot-pro` remote pointing at indica-facil/chatwoot-pro
```
...
>
> ```sh
> gh repo set-default indica-facil/chatwoot       # CE merge → PR/CI on the CE fork
> gh repo set-default indica-facil/chatwoot-pro   # Pro merge → PR/CI on the Pro repo
> ```
>
...
For **CE→Pro merges**, `.annotaterb.yml` is **CO**: adopt CE's newer format but keep the Pro-only `indica_facil/app/models` entry in `model_dir`. Pro scans indica-facil-specific models living under `indica_facil/app/models`; dropping that path silently stops annotation for those models.
...
# Pro-only specs live under indica_facil/spec/, NOT spec/lib/indica_facil/ — rspec silently returns 0 examples on the wrong path
bundle exec rspec indica_facil/spec/lib/indica_facil/integrity_report_spec.rb indica_facil/spec/lib/indica_facil_hub_spec.rb  # run when Pro-specific Ruby touched
