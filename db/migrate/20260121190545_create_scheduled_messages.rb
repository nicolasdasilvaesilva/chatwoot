class CreateScheduledMessages < ActiveRecord::Migration[7.1]
  # Columns (name => cast type) the indica-facil fork table has had since creation.
  ...
  def relocate_conflicting_table
    if fork_table?
      raise ActiveRecord::MigrationError, <<~MSG.squish
        scheduled_messages already exists with the indica-facil fork schema, but migration
