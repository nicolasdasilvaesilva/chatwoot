...
Downstream forks (e.g. `indica-facil/chatwoot-pro`) that mirror a CE release must declare it with a blockquote at the top of each user-notes block, inside the markers. List all mirrored CE versions when there's more than one. CE releases never carry this marker.
...
If nothing qualifies, the blockquote alone is a valid block body. See `v4.14.0-indica-facil.74` for reference.
